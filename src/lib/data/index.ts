...
import { ProductCategoryWithChildren, ProductPreviewType, cmsSkeleton } from "types/global"

import { medusaClient, contentfulClient } from "../config"
...
export async function getCategoriesListByParent(
  offset: number = 0,
  limit: number = 10,
  ...
  const api = new URL(`${MEDUSA_BACKEND_URL}/store/product-categories/filter`)
  api.searchParams.set("offset", offset.toString())
  api.searchParams.set("limit",limit.toString())
  if(parentID){
    api.searchParams.set("parent_category_id", parentID)
  }
...
export async function getCategoryByID(
  ...
  product_category: ProductCategoryWithChildren
}> {
  const { product_category } = await medusaClient.productCategories
    .retrieve(id)
    .catch((err) => {
      throw err
    })
...
export async function getCategoryByHandle(categoryHandle: string[]): Promise<{
  ...
  for (const handle of handles) {
    const category = await medusaClient.productCategories
      ...
      .then(({ product_categories: { [0]: category } }) => {
        if(!category) throw new Error("Handle not found")
        return category
      })
      .catch((err) => {
...
export async function listCMSContents(countryCode: string): Promise<{name: string, handle: string}[]> {
  if(!countryCode){
    return [];
  }
  return contentfulClient.getEntries<cmsSkeleton>({
    content_type: 'cms',
    'fields.region':countryCode
  })
  .then(entries =>{
    return entries.items
    .map((content) => ({ name: content.fields.name, handle: content.sys.id }))
  })
  .catch(e => {
    throw e
  })
